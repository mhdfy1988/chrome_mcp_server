...
export class BrowserSession {
  ...
  private readonly elementRefs = new Map<string, Map<string, string>>();
  ...
  public async closePage(pageId?: string): Promise<BrowserStatus> {
    ...
    this.elementRefs.delete(resolvedPageId);
  ...
  public async syncPages(): Promise<void> {
    ...
    for (const [pageId] of this.pages.entries()) {
      if (!openPageIds.has(pageId)) {
        ...
        this.elementRefs.delete(pageId);
  ...
  public trackPage(page: Page): string {
    ...
    this.elementRefs.set(pageId, this.elementRefs.get(pageId) ?? new Map());

    return pageId;
  }

  public attachElementRefs<T extends Pick<SnapshotElementSummary, "tag" | "selector">>(
    pageId: string,
    elements: T[],
  ): Array<T & { ref: string }> {
  ...
  public resolveSelectorForRef(pageId: string, ref: string): string {
    const pageRefs = this.elementRefs.get(pageId);
    const selector = pageRefs?.get(ref);
    if (!selector) {
      throw new Error(
