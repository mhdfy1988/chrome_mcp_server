...
import type { OverlayBlockingSummary } from "../state/types.js";

export interface BlockingOverlayTargetSummary extends BindingAttachableElement {}

export interface BlockingOverlayDismissCandidate
  extends BlockingOverlayTargetSummary {
...
export interface BlockingOverlayDetectionResult {
  ...
  primaryTarget?: BlockingOverlayTargetSummary;
...
export async function detectBlockingOverlays(
  ...
  return evaluateWithDomHelpers(
    ...
    (helpers, args) => {
      ...
      const summarizeBindingTarget = (
        element: Element,
      ): BlockingOverlayTargetSummary => {
      ...
      const overlayElements = Array.from(document.querySelectorAll("*"))
        ...
        .map((element) => {
          ...
          const isExplicitDialog = element.matches(
          ...
          const insideHeaderChrome = Boolean(
            element.closest("header, nav, [role='banner']"),
          );
          const weakOverlaySignalOnly =
            ...
            !hasLoginLikeInputs &&
            !containsForm;
          const tooSmallToBlock =
            rect.width <= Math.max(120, viewportWidth * 0.24) &&
            rect.height <= Math.max(72, viewportHeight * 0.1);
          if (weakOverlaySignalOnly && (tooSmallToBlock || insideHeaderChrome)) {
            return null;
          }
          ...
          const controls = Array.from(
            ...
            .map((control) => {
              ...
              return {
                ...summarizeBindingTarget(targetControl),
          ...
          return {
            ...
            target: summarizeBindingTarget(element),
          };
        })
        .filter(Boolean)
        .sort((left, right) => {
